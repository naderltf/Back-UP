...
class Backend_mesure:
    ...
    """
          print('brgute motat devnt cuise st',braguette)
          X_cuise = braguette[0]
          Y_cuise = braguette[1]
    ...
    def draw_points(self,opencvMiddle,code_piece,lst,img,s_folder): 
        ...
        original_longuu_detected_by_model = longu
        ...
        print("beeeeeellllt",left_side_belt)    
        print("beeeeeellllt",belt_left_extreme) 
        indxing_left_belt_error = False
        indxing_right_belt_error = False
        try : 
            X_LB, Y_LB = left_side_belt[0][::-1]
        except :
            indxing_left_belt_error = True
            print("Indexing Error left")
        try : 
            
            X_RB, Y_RB = right_side_belt[0][::-1]
        except :
            print("Indexing Error right")
            indxing_right_belt_error = True
        ...
        """
        if not indxing_right_belt_error and not indxing_left_belt_error:
        ...
        if not indxing_right_belt_error and not indxing_left_belt_error:
            ...
            left_side_belt = belt_left_extreme
            right_side_belt = belt_right_extrem
        ...
        print("dcttt _ cuissseee \n",dct_cuise," baaaam cuuiseee")
        ...
        #fixing x of longu :
        try :
            if np.abs(entre_jambe[0]-longu[0])> 20 :
                longu = [original_longuu_detected_by_model[0],longu[1]]
                longu_measure=dist.euclidean(left_side_belt,longu)/10.12
                tour_de_bas=dist.euclidean(entre_jambe,longu)/10.148536951354489
            else:
                longu = longu
        ...
        img = cv2.circle(image,original_longuu_detected_by_model, 2, (0,255,255), 2)
